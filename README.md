...
The dataset used in this project comprises monthly tourist arrival figures in Sri Lanka from January 2014 to October 2024. The data is sourced from [GitHub](https://raw.githubusercontent.com/dev-achintha/Sri_Lanka-Tourism_Forcasting_Model/prophet/dataset/2014-2024-monthly-tourist-arrivals-sl-csv.csv) and includes the following columns:
...
├── images/
│   ├── forecast_combined.png
│   ├── forecast_prophet.png
│   ├── forecast_sarima.png
│   ├── forecast_exponential_smoothing.png
│   ├── forecast_lstm.png
│   ├── residuals_prophet.png
│   ├── residuals_sarima.png
│   ├── residuals_exponential_smoothing.png
│   ├── residuals_lstm.png
...
- **dataset/**: Contains the CSV file with tourist arrivals data.
- **images/**: Stores all visualization plots generated from the analysis.
- **notebooks/**: Jupyter Notebook implementing the forecasting models and analysis.
...
1. **Clone the Repository**
   ...
   ```bash
   git clone https://github.com/your-username/Sri_Lankan_Tourism_Forecasting.git
   cd Sri_Lankan_Tourism_Forecasting
   ```
...
- **Data Loading**: The dataset is loaded from a GitHub URL using `pandas`.
...
To replicate the forecasting and analysis:

1. **Ensure Dependencies Are Installed**: Refer to the [Installation](#installation) section.

2. **Run the Jupyter Notebook**:

   Navigate to the `notebooks` directory and open `Sri_Lankan_Tourism_Forecasting.ipynb` using Jupyter Notebook.
...
3. **Execute Cells Sequentially**:
   ...
   - Execute each cell in order to preprocess data, train models, perform cross-validation, combine forecasts, and visualize results.

4. **Exporting Results**:

   - The notebook includes commands to convert it to PDF and HTML using `nbconvert`.

   ```bash
   jupyter nbconvert --to pdf notebooks/Sri_Lankan_Tourism_Forecasting.ipynb
   jupyter nbconvert --to html notebooks/Sri_Lankan_Tourism_Forecasting.ipynb
   ```

   *Note: Before exporting, ensure that all plots are saved in the `images/` directory to display correctly in the exported documents.*

## Contributing

Contributions are welcome! If you'd like to enhance the forecasting model, fix bugs, or improve documentation, please follow these steps:
...
- **Data Source**: [Sri Lanka Tourism Data](https://raw.githubusercontent.com/dev-achintha/Sri_Lanka-Tourism_Forcasting_Model/prophet/dataset/2014-2024-monthly-tourist-arrivals-sl-csv.csv).
...
## Adding Visualizations to the README

To include the plots in your README, follow these steps:

### 1. Generate and Save Plots

Ensure that each plot in your Jupyter Notebook is saved as an image file (e.g., PNG format) in the `images/` directory of your repository. You can modify your plotting code to save each figure using `plt.savefig()` before `plt.show()`. For example:

```python
# Example for saving the combined forecast plot
plt.figure(figsize=(15, 10))
# ... [your plotting code] ...
plt.savefig('images/forecast_combined.png')
plt.show()
```

Repeat this step for each plot you wish to include in the README, ensuring each image has a descriptive filename.

### 2. Upload Images to the Repository

After saving the plots:

1. **Add the Images to Git**:
...
2. **Commit the Images**:
   ...
   ```bash
   git commit -m "Add visualization plots to images directory"
   ```
...
### 3. Reference Images in the README

In your `README.md`, use the following Markdown syntax to embed images:

```markdown
![Description of Image](images/your_image_filename.png)
```

Ensure the path to the image is correct relative to the README file.

### 4. Example Integrations in README

Refer to the [Visualizations](#visualizations) section in the README above. Here's how each plot is embedded:
...
### 5. Final README with Embedded Images

Below is the complete `README.md` with embedded images for visualization:
